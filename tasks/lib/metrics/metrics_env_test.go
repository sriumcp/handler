...
var _ = Describe("metrics library", func() {
	Context("when running a metrics/collect task", func() {
		...
		It("should initialize an experiment", func() {
			...
			exp, err = (&tasks.Builder{}).FromFile(tasks.CompletePath("../../../", "testdata/metricscollect/metricscollect.yaml")).Build()
		...
		It("should initialize an experiment", func() {
			By("reading the experiment from file")
			exp, err = (&tasks.Builder{}).FromFile(tasks.CompletePath("../../../", "testdata/metricscollect/loadgen.yaml")).Build()
			Expect(err).ToNot(HaveOccurred())
